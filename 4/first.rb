...
class RollFinder
  ...
  def count_free_rolls
    ...
    @grid.each_index do |row|
      @grid[row].each_index do |column|
        if count_neighbouring_rolls(row, column) < TOO_MANY_ROLLS
          result += 1
          @new_grid[row][column] = 'x'
        end
      end
    end
  ...
  # The problem has to do with literal edge cases -- the tiles around the edge of the grid
  def count_neighbouring_rolls(row, column)
    ALL_OFFSETS.reduce(0) do |acc, (row_offset, column_offset)|
      if valid_row?(row + row_offset) &&
         valid_column?(row + row_offset, column + column_offset)
        @grid[row + row_offset][column + column_offset] == '@'
        acc + 1
      else
        acc
      end
  ...
  # These cannot use only nil, they must test for not nil and not negative number
  def valid_row?(row)
    row.positive? && @grid[row]
  end

  def valid_column?(row, column)
    column.positive? && @grid[row][column]
  end
end
